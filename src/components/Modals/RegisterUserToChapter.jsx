...
const RegisterUserToChapter = ({close}) => {
    ...
    const chaptersList = queryClient.getQueryData('all-chapters').results || undefined

    const {isLoading:userLoading, isFetching:userFetching, isError:userError, data:userData} = useQuery('all-users', getAllMembers, {
      ...
      const {isLoading: createLoading, mutate} = useMutation(registerUserToChapter, {
        onMutate: () => {
            toast.info("User Registration in progress",{progressClassName:"toastProgress",icon:false})
        },
        onSuccess: () => {
            toast.success("User Registered",{progressClassName:"toastProgress",icon:false})
    ...
    return (
        <BackDrop>
                ...
                {(userLoading || userFetching) ? <Loading loading={userLoading || userFetching}/> : (!userError) ? 
                    <SubCon>
                        <SubConHeader>Register User To Chapter</SubConHeader>
                        <Form onSubmit={handleSubmit(onSubmit)}>
                            <FormLabel>
                                 User:
                                <FormSelection defaultValue={""} {...register("user_id", {required:true})}>
                                    <FormOption disabled value="">select an option</FormOption>
                                    {
                                        userData.map(item => (
                                            <FormOption key={item.user} value={item.user}>{item.user} | {item.email}</FormOption>
                                        ))
                                    }
                                </FormSelection>
                            </FormLabel>
                            ...
                            <FormLabel>
                                 Chapter:
                                <FormSelection defaultValue={""} {...register("chapter_id", {required:true})}>
                                    <FormOption disabled value="">select an option</FormOption>
                                    {
                            ...
                            <SubConBtnHold>
                                <SubConBtn type={"submit"} disabled={createLoading} value="Add" typex="filled"/>
                                <SubConBtn type={"submit"} disabled={createLoading} value="Cancel" onClick={close}/>
                            </SubConBtnHold>
                        </Form>
                    </SubCon>
