...
const AddCommittee = ({close}) => {
    ...
    const {fields:dutyFields, append:dutyAppend, remove:dutyRemove} = useFieldArray({
        name: "commitee_duties.commitee_duties",
        control
    })

    const { isLoading, isFetching, isError, data } = useQuery("all-members",getAllMembers, {
    ...
    const {isLoading: createLoading, mutate} = useMutation(createCommittee, {
        onMutate: () => {
            toast.info("Committee Creation in progress",{progressClassName:"toastProgress",icon:false})
        },
        onSuccess: () => {
            toast.success("Committee Created",{progressClassName:"toastProgress",icon:false})
    ...
    const onSubmit = (data) => {
        let { team_of_reference, commitee_todo, commitee_duties, members_list, name } = data
        ...
        {(isLoading || isFetching) ? <Loading loading={isLoading || isFetching}/> : (!isError) ? 
            <SubCon>
                ...
                <Form onSubmit={handleSubmit(onSubmit)}>
                    ...
                    <FormLabel>
                        Members List:
                        <FormSelection multiple defaultValue={[]} {...register("members_list", {required:true})}>
                    ...
                    <DeleteButton type='button' mt="filledup" onClick={() => append("New Todo")
                    }>Add Committee Todo</DeleteButton>
                        ...
                        dutyFields.map((field, index)=>{
                            return(
                                <section key={field.id}>
                                    <FormLabel>
                                        Committee Duty:
                                        <FormDataComp type={"text"} {...register(`commitee_duties.commitee_duties.${index}`, {required:true})}/>
                                    </FormLabel>
                                        <DeleteButton typex="filled" type='button' onClick={() => dutyRemove(index)}>Delete</DeleteButton>
                                </section>
                            )
                        })
                    }
                    <DeleteButton type='button' mt="filledup" onClick={() => dutyAppend("New Duty")
                    }>Add Committee Duty</DeleteButton>
                    

                    <SubConBtnHold>
                        <SubConBtn type={"submit"} value="Add" disabled={isLoading || createLoading} typex="filled"/>
                        <SubConBtn type={"submit"} value="Cancel" disabled={isLoading || createLoading} onClick={close}/>
                    </SubConBtnHold>
                </Form>
            </SubCon>
